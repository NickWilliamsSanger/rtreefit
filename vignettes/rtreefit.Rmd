...
First lets simulate a tree using rsimpop:
...
We need to associate an "age" with each sampled tip:

```{r}
st$agedf=data.frame(tip.label=sprintf("s%d",1:31),age=c(1e-6,rep(5,30)))
```

We then fit a model with the early growth switched model switched off
```{r}
res=fit_tree(tree=st,switch_nodes = c(),xcross = c(),niter = 10000,model = "poisson_tree",early_growth_model_on = 0.0)
print(res$lambda)
```

This also gives us an ultrametric tree
```{r}
par(mfcol=c(1,2))
ttree=get_elapsed_time_tree(st)
ttree$edge.length=ttree$edge.length/365
plot_tree(get_elapsed_time_tree(st),cex.label=0.5);title("True Real Time Tree")
plot_tree(res$ultratree,cex.label=0.5);title("Inferred Real Time Tree")
```
